...
pub async fn artist(_: Context<'_>) -> Result<(), Error> {
    Ok(())
}

#[tracing::instrument(skip_all)]
#[poise::command(prefix_command, guild_only)]
pub async fn add(ctx: Context<'_>, #[rest] name: String) -> Result<(), Error> {
    ...
    .await
    .inspect_err(|e| {
        tracing::error!(err = ?e, name = %name, "an error occurred when adding artist");
    });
        ...
        if e.as_database_error().unwrap().is_unique_violation() {
            ctx.send(
                poise::CreateReply::default()
                    .content(format!("artist with name \"{name}\" already exists.")),
            )
            .await
    ...
    ctx.send(poise::CreateReply::default().content(format!("added artist \"{name}\".")))
        .await
        .inspect_err(|e| tracing::error!(err = ?e, "an error occurred when sending reply"))?;
